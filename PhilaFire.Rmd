...
# Extracting year
dat <- dat %>% mutate(year = year(alarm_date))

# Reducing columns
dat <- dat%>%
  dplyr::select(address, year, zip_code, census_tract,incident_number, number_of_exposures, incident_type, building_status, fire_spread, minor_damage, significant_damage, heavy_damage, extreme_damage, no_spread, code_description, geometry )

# Initial panel
dat.panel <- 
  expand.grid(year = unique(dat$year), 
...
# Final Panel
final.panel <- full_join(dat.panel, count.panel, by=c("address", "year")) # Join
final.panel <- final.panel %>% dplyr::select(address, year, n.y) %>% rename(count = n.y) # Condensing & Renaming
final.panel[is.na(final.panel)] <- 0 # Assigning NA to 0

# Joining the other data
final.panel_TEST <- left_join(final.panel, dat, by=c("address", "year")) # Join
