...
# test311 <- LI%>%
#   #filter(grepl("Vacant Property", subject, fixed = TRUE))%>%
#   #filter(agency_responsible == "License & Inspections")%>%
#   filter(requested_datetime > 2020-01-01)%>%
#     dplyr::select(subject, objectid, service_request_id, status, service_name, service_code, requested_datetime, agency_responsible, address, zipcode, lat, lon)%>%
...
table(property311$subject)

g(vacantLI_count)
filter(vacantLI_count, address == "1756 N TANEY ST")
...
# vacant311_count%>%
#   group_by(quarter, service_name)%>%
#   summarize(count = sum(n_311Vacant))%>%
#   ggplot(aes(x=quarter, y=count, fill=service_name))+
#   geom_col(position = "dodge")+
#     labs(title="Frequency Vacancy Reports",
#        subtitle="Philadelphia County, 2009-2022, Above 50 Unique Incidents")+
#   xlab("Date, Rounded to Beginning of Quarter")+
#   ylab("Number of Fires")+
#   theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
panel_OPAFire311 <- left_join(opa_count.panel, vacantLI_count, by=c("address", "quarter"))

panel_OPAFire311$n_LIVacant[is.na(panel_OPAFire311$n_LIVacant)] <- 0
...
ggplot()+
  geom_sf(data=acsTractsPHL.2020, fill='#f0efe0', color='dark gray')+
  geom_sf(data=opaFireVacant_PlotData, color='red', alpha=0.5)+
