...
panel_FirePositivesDiff%>%
  ...
  dplyr::select(n_Vacant, n_permits, n_transfers, mSinceFire)%>%
  gather(Variable, value, -mSinceFire)%>%
  filter(value > 0)%>%
  group_by(Variable)%>%
  summarize(`Median Months Since Fire` = median(mSinceFire))%>%
  kable()%>%
  kable_styling()
...
panel_Results <- panel_OPAFireOpenData %>%
