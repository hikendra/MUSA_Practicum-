...
vacantLI_count%>%
  group_by(quarter)%>%
  summarize(count = sum(n_LIVacant))%>%
  ggplot(aes(x=quarter, y=count))+
  geom_col(fill="#A5300F")+
...
vacantLI311 <- rbind(vacantLI_count, vacant311_count)
g(vacantLI311)
...
table(permits1623$commercialorresidential)
...
#if fire or outcome, then keep address
#if outcome, then outcome time minus fire time
#Average outcome time
...
panel_FirePositives <- panel_OPAFireOpenData %>%
  filter(count > 0)%>%
  select(address, severity_index)%>%
  distinct(address, .keep_all = TRUE)%>%
  rename(fire_spread = severity_index)%>%
...
#only start with fires
panel_FirePositives <- panel_FirePositives%>%
  group_by(address)%>%
  mutate(f = cumsum(count))%>%
  filter(f > 0)%>%
  dplyr::select(-f)
...
panel_FirePositivesDiff <- panel_FirePositives%>%
  ...
  mutate(mSinceFire = interval(quarter.y, quarter.x) %/% months(1),
         ...
         code_descr = toupper(category_code_description))%>%
  filter(mSinceFire >= -1, 
         mSinceFire < 49)%>%
  select(-category_code_description)%>%
  st_as_sf()

panel_FirePositivesDiff$fire_spread <- as.factor(panel_FirePositivesDiff$fire_spread)
...
#1 year code
panel_Results1Y <- panel_OPAFireOpenData %>%
    mutate(fireVacant2Q = ifelse(address == lag(address, n=1) & 
                               (count > 0 | lag(count, n=1) > 0) &
                                (n_LIVacant > 0) > 0 , 1, 0),
           firePermit2Q = ifelse(address == lag(address, n=1) & 
...
#Distribution of Outcomes, Measuring Time between Fire abd Outcome
panel_FirePositivesDiff%>%
  ungroup()%>%
  filter(!(n_Vacant == 0 & n_permits == 0 & n_transfers == 0))%>%
  dplyr::select(n_Vacant, n_permits, n_transfers, ySinceFire)%>%
...
panel_FirePositivesDiff%>%
  ungroup()%>%
  filter(n_permits == 1)%>%
  dplyr::select(code_descr, ySinceFire)%>%
  gather(Variable, value, -ySinceFire)%>%
  filter(value > 0)%>%
...
#Measuring Time between fire and Real Estate Transfers, By Severity
panel_FirePositivesDiff%>%
  ungroup()%>%
  filter(n_transfers == 1)%>% #change this line to switch variables
  dplyr::select(fire_spread, ySinceFire)%>%
  gather(Variable, value, -ySinceFire)%>%
  ggplot(aes(x=ySinceFire, fill=value))+
    geom_bar(position="dodge")+
      labs(title="Time Between Real Estate Transfer and Fire, by Severity",
       subtitle="Philadelphia County, 2009-2022")+
  xlab("Years from Fire (Rounded to Quarter)")+
  ylab("Count")+
  scale_fill_manual(values=palette6_sat)+
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
    panel.background = element_rect(fill = "#f3efe0"))
...
ggplot()+
  ...
  geom_sf(data=panel_FirePositivesDiff, aes(color=ySinceFire))
