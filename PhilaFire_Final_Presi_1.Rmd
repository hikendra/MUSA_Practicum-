...
opa_dat_small_sf <- opa_dat[!duplicated(opa_dat$location),] %>%
  dplyr::select(location, category_code, category_code_description, building_code, building_code_description, building_code_new, building_code_description_new, total_area, total_livable_area, owner_1, owner_2, market_value, market_value_date, number_of_bedrooms, number_of_bathrooms, number_stories, interior_condition, assessment_date, year_built, year_built_estimate, zoning, quality_grade, central_air, exterior_condition, fireplaces, fuel, taxable_building, topography, type_heater, sale_price, separate_utilities)%>%
...
fire_panel <-
  fire_panel %>%
  mutate(built_year = case_when( # 1988 is when fire detectors became mandatory
    year_built < 1988 ~ "Optional", # Prior to 1988
    TRUE ~ "Mandatory")) # 1988 and after
...
QUESTION
- so will we need three GLM models and three xgb models?
```{r Initial Split for Training and Test}
# Vacant
split_vacant <- initial_split(fire_panel_nogeo, strata = "vacant", prop = 0.75)
train_vacant <- training(split_vacant)
test_vacant <- testing(split_vacant)

# Permit
split_permit <- initial_split(fire_panel_nogeo, strata = "permit", prop = 0.75)
train_permit <- training(split_permit)
test_permit <- testing(split_permit)

# Transfer
split_transfer <- initial_split(fire_panel_nogeo, strata = "transfer", prop = 0.75)
...
# GLM Models
QUESTION
- what would our dependent variable be? the outcome or time? for example: vacant or mSinceFire 
```{r Vacant }
...
QUESTION
- how would we interpret the predictions?  - what is the probablity of x within two years
```{r Permit}
# Do not add livable area, market value, or incident type
...
```{r Transfer}
# Do not add livable area, market value, or incident type
model_transfer <- glm(as.factor(transfer) ~ severity_index + outcome_vacant + outcome_permit + outcome_transfer + quarter + 
                        number_stories + exterior_condition + grade + fuel_type + topo + electric_heater + air_central + 
                        fireplace + mSinceFire + cat_code + value + mapname +  bedrooms + bathrooms + area + built_year + 
                        condition,
                  family="binomial"(link="logit"), data = train_transfer)

# severity_index + outcome_vacant + outcome_permit + outcome_transfer + quarter + number_stories + exterior_condition + grade + fuel_tyoe + topo + electric_heater + air_central + fireplace + mSinceFire + cat_code + value + mapname + stations_nn1 + stations_nn2 + stations_nn3 + stations_nn4 + stations_nn5 + bedrooms + bathrooms + area + built_year + condition
