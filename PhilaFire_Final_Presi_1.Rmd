...
model_permit_2 <- glm(permit ~ severity_index + exterior_condition + air_central + 
                        cat_code + resident_type + poverty + value_sqft + income_level ,
                  family="binomial" (link="logit"), data = train_permit) # Best model so far

summary(model_permit_1)
```

```{r}
classProbs <- predict(model_permit_1, train_permit, type="response")
hist(classProbs)

testProbs <- data.frame(Outcome = as.factor(test_permit$permit),
                        Probs = predict(model_permit_1, test_permit, type= "response"))
testProbs <-
  testProbs %>%
  na.omit()
...
```{r Transfer}
model_transfer_1 <- glm(transfer ~ severity_index + exterior_condition + air_central + 
                        cat_code + resident_type + poverty + value_sqft + income_level,
                  family="binomial" (link="logit"), data = train_transfer)
```

```{r}
classProbs <- predict(model_transfer_1, train_vacant, type="response")
hist(classProbs)
